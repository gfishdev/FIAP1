package br.com.fiap.persistenciaEmJava.aula2.roteiro3.helper;
...
import br.com.fiap.persistenciaEmJava.aula2.roteiro3.config.HibernateUtil;
import br.com.fiap.persistenciaEmJava.aula2.roteiro3.entity.Forum;
import br.com.fiap.persistenciaEmJava.aula2.roteiro3.entity.Usuario;
...
public class ForumHelper {
	...
	public List<Usuario> listarUsuariosCriteria() {
		...
		try {
			session = HibernateUtil.getSessionFactory().getCurrentSession();
			usuarios = session.createCriteria(Usuario.class).add(Restrictions.in("nome", "paulo"))
					.add(Restrictions.disjunction().add(Restrictions.isNotNull("email"))
	...
	public List<Usuario> listarUsuariosHql() {
		...
		try {
			session = HibernateUtil.getSessionFactory().getCurrentSession();
			Query query = session
					.createQuery("from Usuario where nome = :nome and (email is not null and email like :email) ");
	...
	public List<Usuario> listarUsuariosNative() {
		...
		try {
			session = HibernateUtil.getSessionFactory().getCurrentSession();
			Query queryN = session
					.createSQLQuery(
							"select * from usuario u where u.nome = :nome and u.email is not null and email like :email")
					.addEntity(Usuario.class).setParameter("nome", "paulo").setParameter("email", "%@%");
